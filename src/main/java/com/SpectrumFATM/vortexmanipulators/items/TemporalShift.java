...
import com.SpectrumFATM.vortexmanipulators.network.PacketTemporalShift;
...
public class TemporalShift extends Item implements IItemTooltipProvider {

    public TemporalShift(Properties p_i48487_1_) {
        super(p_i48487_1_);
    }
    ...
    public ActionResult<ItemStack> use(World world, PlayerEntity player, Hand hand) {
        if (player.isCrouching()) {
            if (world.isClientSide) {
                player.displayClientMessage(new StringTextComponent("===TS===\nEmergency temporal shift initiated!").withStyle(TextFormatting.BOLD).withStyle(TextFormatting.RED), false);
                NetworkHandler.INSTANCE.sendToServer(new PacketTemporalShift());
            }
        }
    ...
    public void createDescriptionTooltips(ItemStack stack, World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        tooltip.add(new StringTextComponent("A single-use Dalek emergency temporal shift circuit. Shift-right click to shift to a random location, but beware... Time travel without a capsule, it's a killer.").withStyle(TextFormatting.GRAY));
    }
